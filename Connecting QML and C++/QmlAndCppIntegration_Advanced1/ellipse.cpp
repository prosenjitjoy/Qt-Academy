...
float Ellipse::border() const { return m_border; }

void Ellipse::setBorder(float newBorder) {
  if (qFuzzyCompare(m_border, newBorder)) return;
...
void Ellipse::shapeInfo() { ellipseInfo(); }

void Ellipse::ellipseInfo() {
  qInfo()
      << QString("Ellipse's color: %1 - Ellipse's border: %2").arg(m_color.name()).arg(m_border);
}

void Ellipse::onPropertyChanged() { update(); }
